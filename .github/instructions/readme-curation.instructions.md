...
- Maintain alphabetical order within sections where practical.
- Avoid hype, superlatives, and unverifiable claims.
- Avoid unsafe biological procedural detail of any kind.
- No duplicate entries; no dead links.
- Optional plain-text tags (for example `` `paper` `` `` `model` ``) only where they aid scanning. Do not over-tag.
- Do not add scoring tables, long analysis, or paragraph entries to `README.md`.
